...
#define TIMER_IO_INIT_NAME "File I/O Init"
...
int main(int argc, char** argv) {
  try {
    ...
    TIME(filesystem::Init(), TIMER_IO_INIT_NAME);
    ...
    for (const auto& [name, duration] : telemetry::GetTimings()) {
  ...
  try {
    while (glfwWindowShouldClose(render::g_window) == 0) {
      ...
      if (filesystem::g_level != nullptr) {
        TIME(filesystem::g_level->Update(), TIMER_LEVEL_UPDATE_NAME);
      }
      TIME(physics::Update(), TIMER_PHYSICS_UPDATE_NAME);
      TIME(render::Update(), TIMER_RENDER_UPDATE_NAME);
      if (filesystem::g_level != nullptr) {
        TIME(filesystem::g_level->Render(), TIMER_LEVEL_RENDER_NAME);
      }
